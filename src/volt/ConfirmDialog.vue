    ...
    >
        <template #container="{ message, acceptCallback, rejectCallback }">
            ...
            <div class="pt-0 px-5 pb-5 flex justify-end gap-2">
                <SecondaryButton :class="message.rejectProps.class" :label="message.rejectProps.label" size="small" @click="rejectCallback" />

                <Button :class="message.acceptProps.class" :label="message.acceptProps.label" size="small" @click="acceptCallback" />
            </div>
        </template>
    </ConfirmDialog>
</template>
