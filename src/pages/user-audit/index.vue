...
<template>
<LayoutNormalCenter page-name="User Audit"
  page-description="Search for school district tenants by name, dashboard subdomain or tenant id. Click on a row for details about the district tenant.">

  <DataTable v-model:filters="filters" :value="tenants" paginator :rows="10" data-key="id"
    :global-filter-fields="['name', 'domain', 'id']">
    <template #header>
    ...
    <Column field="id" header="" :show-filter-menu="false">
      <template #body="{ data }">
        <router-link :to="'/user-audit/' + data.id">
