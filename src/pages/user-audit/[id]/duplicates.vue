...
<div>
  <div class="flex flex-1 items-center justify-left p-2">
    ...
    <div class="flex flex-col gap-2 pl-7 ml-7 items-center">
      <div class="text-lg font-medium text-surface-900 dark:text-surface-0">
        Ignore Middle Names:
      </div>
      <div>
        <ToggleSwitch id="ignoreMiddleNamesToggle" v-model="userStore.duplicatesIgnoreMiddleName" />
      </div>
    </div>
    <div class="flex flex-col gap-2 pl-7 ml-7 items-center">
      <div class="text-lg font-medium text-surface-900 dark:text-surface-0">
        Only Show Duplicates for Same Child:
      </div>
      <div>
        <ToggleSwitch id="sameChildDuplicatesToggle" v-model="userStore.onlyShowDuplicatesForSameChild" />
      </div>
    </div>
  </div>

  <div v-if="userStore.getUsersWithDuplicatedNames" class="pt-5">
    <DefaultUserAgGrid :loading="userStore.loading" :users-with-role-names="userStore.getUsersWithDuplicatedNames"
      :tenant-id="`${route.params.id}`" :is-duplicates-tab="true"
      :loaded-user-count="userStore.activeTenantUsers?.size || 0"
